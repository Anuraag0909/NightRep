/**
 * Deploy reputation-system contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
// reputation contract has no witnesses, so its private state is empty ({}).
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'reputation-state',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy reputation-system to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState (empty here — the reputation contract has no
      // witnesses). args is the contract constructor's arguments: empty for
      // reputation's no-arg constructor. (Statically-typed contracts can omit
